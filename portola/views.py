...
class DocumentViewSet(
    ...
    @action(detail=True, methods=['post'],
        permission_classes=(IsProjectApprover|IsSuperUser,),
        serializer_class=DocumentDisclosureSerializer)
    def update_status_to_pending(self, request, pk=None):
        self.context = {'request': request}
        disclosure = request.data.get('disclosure')  # Get the desired disclosure from the request data
        if disclosure:
            queryset = Document.objects.get(id=pk)
            if queryset.disclosure != disclosure:
                disclosure_date = datetime.now().date()
                queryset.disclosure = disclosure
                queryset.disclosure_date = disclosure_date
                queryset.save()
                serializer = DocumentSerializer(queryset, many=False, context=self.context)
                return Response(serializer.data)
            else:
                return Response("Document is already in the specified disclosure.")
        else:
            return Response("Disclosure not provided in the request.")
...
class UserViewSet(LoggingMixin, viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if is_active is not None:
            is_active_val = is_active.lower() in ['true']
            queryset = queryset.filter(is_active=is_active_val)

        if is_staff is not None:
            is_staff_val = is_staff.lower() in ['true']
            queryset = queryset.filter(is_staff=is_staff_val)
